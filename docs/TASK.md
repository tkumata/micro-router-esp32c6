# XIAO ESP32C6 マイクロ Wi-Fi ルータ - 実装タスク
...
void setup() {
  ...
  Serial.println("Web サーバ起動: http://192.168.4.1");
}
...
- AP に接続後、ブラウザで http://micro-router.local にアクセス
- Web ページが表示されることを確認
...
void handleRoot() {
  ...
  html += "<h1>XIAO ESP32C6 マイクロルータ</h1>";
...
**原因**: Web サーバが起動していない、または IP アドレスが違う
...
- [x] DHCP オプションで ESP32C6 自身を DNS サーバとして広告
- [x] AP クライアントが ESP32C6 を DNS サーバとして使用するよう設定
...
**原因**: クライアントが ESP32C6 を DNS サーバとして使用していない

**確認事項**:

- DHCP サーバーが ESP32C6 (192.168.4.1) を DNS サーバとして広告しているか
